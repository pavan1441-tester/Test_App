...
public class AdvancedBulkSchedulerSitesPage extends ComboAppBasePage {

	public AdvancedBulkSchedulerSitesPage() 
	{
		super();
	}
		//Goto Sites module
		...
		@FindBy(css=".pl20 > .checkbox-custom-label")	
	    private WebElement selectAllSitesCheckbox;
		
	   //Click on New bulk scheduler green button
		@FindBy(css=".light-green > .fa")	
	    private WebElement newBulkscheduleButton;
	    ...
	    private WebElement selectTradesmanName;
		...
		public void fnAdvancedBulkSchedulerSites(int iTestCaseID, UCAppData testData) throws Exception 
		{
		   ...
		   waitForThread(CommonClass.iWaitForThread);
			...
			Mwoname.sendKeys("Sites automated WO scheduler");
			waitForThread(CommonClass.iWaitForThread);
			...
			Descriptiondetl.sendKeys("Advanced bulk WO scheduler -Automation");
			waitForThread(CommonClass.iWaitForThread1);
		...
		/*	if(!CommonClass.driver.findElements(By.xpath("//*[@id='webWidget']")).isEmpty())
          ...
          else
           {
			...
			selectTradesmanName.click();
			System.out.println("Tradesman selected");
			waitForThread(CommonClass.iWaitForThread);
