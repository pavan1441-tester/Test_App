...
public class SiteCreationPage extends ComboAppBasePage 
{

	public SiteCreationPage() 
	{
		super();
	}
	...
	@FindBy(xpath="//span[@id='select2-location_company_id-container']")
	private WebElement clickOnSelectClientButton;

	//Enter Client Name
	@FindBy(css="body > span > span > span.select2-search.select2-search--dropdown > input")
	private WebElement enterClientName;

	//Click on Entered Client Name
	@FindBy(css="#select2-location_company_id-results > li")
	private WebElement selectEnteredClientName;
	...
	@FindBy(xpath="//*[@id='active-sites']/table[1]/tbody/tr/td/div[3]/a")
	...
	private WebElement siteUploadLogo;
	...
	//Add new Functions
	public void fnSiteCreation(int iTestCaseID, UCAppData testData) throws Exception
	{
	...
	waitForThread(CommonClass.iWaitForThread);
	clickOnSelectClientButton.click();
	waitForThread(CommonClass.iWaitForThread2);
	 
	sendValue(enterClientName,"core","Enter the Existing Comapny Name");
	waitForThread(CommonClass.iWaitForThread);
	 
	waitForThread(CommonClass.iWaitForThread);
	selectEnteredClientName.click();
	waitForThread(CommonClass.iWaitForThread2);
	waitForThread(CommonClass.iWaitForThread);
	clickOnContinueButton.click();
	...
	siteUploadLogo.click();
	waitForThread(CommonClass.iWaitForThread);
	StringSelection siteatttchment= new StringSelection("./Upload/20943580.jpg");
	Toolkit.getDefaultToolkit().getSystemClipboard().setContents(siteatttchment,null);
	...
	//siteUploadLogo.sendKeys("./Upload/20943580.jpg");
	waitForThread(CommonClass.iWaitForThread);
  ...
  else
   {
	...
	List <WebElement> RegionList = CommonClass.driver.findElements(By.xpath("//*[@id='select2-location_cc_location_territory_id-results']/li"));
    Random r11 = new Random();
    int randomValue22 = r11.nextInt(RegionList.size()); //Getting a random value
   RegionList.get(randomValue22).click(); //Clicking on the random item in the list.
	waitForThread(CommonClass.iWaitForThread2);
  ...
  else
   {
	...
	List <WebElement> PropertyMangemList = CommonClass.driver.findElements(By.xpath("//*[@id='select2-location_property_managements_attributes_0_company_id-results']/li"));
    Random r3 = new Random();
    int randomValue3 = r3.nextInt(PropertyMangemList.size()); //Getting a random value
    PropertyMangemList.get(randomValue3).click(); //Clicking on the random item in the list.
	waitForThread(CommonClass.iWaitForThread2);
