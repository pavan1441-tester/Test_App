...
public class CommonClass
{
	...
	public static String SAFARI_DRIVER_PATH;
			...
			int iWaitForElement, int iWaitForThread, int iWaitForThread1, int iWaitForThread2, String sBaseURL)
	{
		...
		//by harish
		Recording_PATH = new File(runner.getOutputDirectory()).getParent() + "_REcording\\";
	...
	{
		System.out.println("Class File path open");
		log.info("Before test launch...");
		log.debug("Testing on browser: " + browser);		
		browser = browser.toLowerCase();
		System.out.println("before open chrome");
		if (browser.equals("chrome"))
		{        
			System.out.println("chrome driver match");
			System.setProperty("webdriver.chrome.driver", CHROME_DRIVER_PATH);
			System.out.println("chrome driver path match");
			driver = new ChromeDriver();
			System.out.println("chrome driver sussessfull");
		}
		else if (browser.equals("firefox"))
		{
			//driver = new FirefoxDriver(downloadPDFFile());
			driver = new FirefoxDriver();
		}
		
		else if (browser.equals("SafariDriver"))
		{
			System.setProperty("webdriver.SafariDriver", SAFARI_DRIVER_PATH);
		...
		//ScreenRecorderUtility.startRecording(Recording_PATH);
